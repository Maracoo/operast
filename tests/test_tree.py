import pytest
from operast.constraints import Partial, Sib, Total
...
class TestCanonicalNormalForm:
    ...
    def test_complex_canonical_nf_B(self):
        ...
        expected = Or(
            Then(Branch("A", "A"), Branch("A", "B"), Branch("A", "C")),
            Then(Branch("A", "A"), Branch("A", "B"), Branch("A", "D")),
        )
        assert result == expected
    ...
    def test_complex_canonical_nf_D(self):
        ...
        expected = And(
            Branch("A", "A", "B"),
            And(Branch("A", "A", "C", "D"), Branch("A", "A", "C", "E")),
        )
        assert isinstance(result, Fork)
        assert result.loc == 2
        assert result == expected
    ...
    def test_complex_canonical_nf_E(self):
        result = Then(
            Or(Branch("A"), Branch("B")), Or(Branch("C"), Branch("D"))
        ).canonical_nf()
        expected = Or(
    ...
    def test_canonical_nf_invariant_only_one_Or(self):
        cnf = And(
            Or("A", Or("B")), Branch("C", Or("D", And("B", Then("E", Or("A", "F")))))
        ).canonical_nf()
...
class TestToExpression:
    ...
    def test_complex_to_expr_2(self):
        tp = Branch(
            "A", "B", And("C", Branch("D", Or("E", Then("F", "G"))), "X")
        ).canonical_nf()
